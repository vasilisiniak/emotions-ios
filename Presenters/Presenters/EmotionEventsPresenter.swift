...
extension EmotionEventsPresenterImpl: EmotionEventsUseCaseOutput {
    ...
    public func presentTrashInfo() {
        output.show(message: "Свайпните запись влево, чтобы восстановить или стереть навсегда\n\nЗаписи хранятся в корзине 3 дня, а затем удаляются автоматически", button: "ОК")
    }

    public func presentDeleteInfo() {
        output.show(message: """
            Запись помещена в корзину — её можно просмотреть, тапнув по иконке в левом верхнем углу\n
            Записи хранятся в корзине 3 дня, а затем удаляются автоматически\n
            Режим корзины можно отключить на вкладке настроек — тогда записи будут стираться сразу при удалении
        """, button: "ОК")
    }
